...
class ApiOrderService
{
    ...
    public function validatorCreateOrder(Request $request): void
    {
        $validator = Validator::make($request->all(), [
            ...
            'buy_amount.required' =>  __('dujiaoka.prompt.buy_amount_format_error'),
            'buy_amount.integer' =>  __('dujiaoka.prompt.buy_amount_format_error'),
            'buy_amount.min' =>  __('dujiaoka.prompt.buy_amount_format_error'),
//            'payway.required' =>  __('dujiaoka.prompt.please_select_mode_of_payment'),
    ...
    public function validatorLoopCarmis(Request $request)
    {
        $carmis = Carmis::query()
            ...
            ->where('is_loop', true)
            ->count();
        if($carmis > 0 && $request->input('buy_amount') > 1){
    ...
    public function validatorChargeInput(Goods $goods, Request $request): string
    {
        $otherIpt = '';
        // 代充框验证
        if (($goods->isSupplierGoods() || $goods->type == Goods::MANUAL_PROCESSING) && !empty($goods->other_ipu_cnf)) {
