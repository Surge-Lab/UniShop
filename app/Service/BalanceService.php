...
class BalanceService
{
     ...
     * @param string $callbackNo 第三方订单号
     * @return bool
     * @throws Exception
     */
    public function completedOrder($tradeNo, $actualPrice, $callbackNo = '')
    {
        // 查找充值订单
        $rechargeOrder = RechargeOrder::where('order_sn', $tradeNo)->first();
        if ($rechargeOrder) {
            return $this->handleRechargeSuccess($rechargeOrder, $callbackNo);
        }

        // 查找普通订单
        $order = \App\Models\Order::where('order_sn', $tradeNo)->first();
        if (!$order) {
