...
class OrderProcessService
{
     ...
     * @var int
     */
    private $isSupplier=0;
    /**
     * 供应商订单号
     * @var string
     */
    private $supplierOrderSn;
     ...
     * @var string
     */
    private $agentOrderSn;
    /**
     * 是否代理订单
     * @var string
     */
    private $agentOrderData;
     ...
     * @link      http://utf8.hk/
     */
    public function createOrder(int $user_id=0): Order
    {
        try {
            $order = new Order();
            ...
            //是否供应商订单
            $order->is_supplier = $this->isSupplier;
            $order->supplier_id = $this->isSupplier?$this->goods->supplier_id:0;
            // 标题
     ...
     * @link      http://utf8.hk/
     */
    public function completedOrder(string $orderSN, float $actualPrice, string $tradeNo = '',$userId=0)
    {
        DB::beginTransaction();
        try {
            // 得到订单详情

            $order = $this->orderService->detailOrderSN($orderSN,$userId);
            if (!$order) {
